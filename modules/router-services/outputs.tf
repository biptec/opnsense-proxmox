...
output "public_dns_ipv6_vip_id" {
  description = "UUID of the public DNS IPv6 IP Alias, or null while detached."
  value       = try(opnsense_interfaces_vip.public_dns_ipv6[0].id, null)
}

output "public_caddy_vip_id" {
  description = "UUID of the public Caddy IPv4 IP Alias, or null while the VIP is detached."
  value       = try(opnsense_interfaces_vip.public_caddy[0].id, null)
}

output "public_caddy_ipv6_vip_id" {
  description = "UUID of the public Caddy IPv6 IP Alias, or null while detached."
  value       = try(opnsense_interfaces_vip.public_caddy_ipv6[0].id, null)
}
