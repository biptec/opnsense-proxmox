...
Public DNS and Caddy addresses are `/32` aliases. The module requires the primary WAN prefix and gateway and verifies that both aliases and the gateway are inside the connected primary WAN subnet. This makes the aliases pure service identities while the primary WAN address supplies gateway reachability and avoids duplicate connected routes.

NTP is configured on its dedicated service interface before either public WAN alias is attached. This ordering prevents a default wildcard NTP configuration from transiently acquiring a newly-created public address. The module exports `service_binding_guard`; additional WAN-identity modules must depend on that output rather than assuming `no_bind` protects every daemon.
